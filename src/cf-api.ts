import type { CfAccount, CfZone, DeployMethod } from './types';
...
/* ------------------------------------------------------------------
 * Admin dashboard (wizard's own D1) — logs successful deployments and
 * lets the wizard owner view/search/delete them at /#admin.
 * ------------------------------------------------------------------ */
...
export interface AdminStats {
  ...
  byMethod: { method: string; c: number }[];
  byDay: { day: number; c: number }[];
  rows: DeploymentRow[];
  limit: number;
  offset: number;
}
...
function adminBase(): string {
  return proxyBase() + '__admin/';
}

// fire-and-forget: never blocks or fails the deploy flow for the user
export function logDeployment(input: DeploymentLogInput): void {
  try {
    fetch(adminBase() + 'log', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    // ignore — logging must never break the wizard
  }
}

export function getAdminKey(): string {
  try {
    return sessionStorage.getItem('ef_admin_key') || '';
  } catch {
    return '';
  }
}
...
export function setAdminKey(key: string): void {
  try {
    if (key) sessionStorage.setItem('ef_admin_key', key);
    else sessionStorage.removeItem('ef_admin_key');
  } catch {}
}

export async function fetchAdminStats(key: string, opts?: { limit?: number; offset?: number; q?: string }): Promise<AdminStats> {
  const params = new URLSearchParams();
  if (opts?.limit) params.set('limit', String(opts.limit));
  if (opts?.offset) params.set('offset', String(opts.offset));
  if (opts?.q) params.set('q', opts.q);
  const r = await fetch(adminBase() + 'stats?' + params.toString(), {
...
export async function deleteDeploymentRow(key: string, id: number): Promise<void> {
  const r = await fetch(adminBase() + 'log/' + id, { method: 'DELETE', headers: { 'X-Admin-Key': key } });
  const data = await r.json();
  if (!r.ok || data.success === false) throw new Error(data.error || `HTTP ${r.status}`);
}

export async function clearAllDeployments(key: string): Promise<void> {
  const r = await fetch(adminBase() + 'log', { method: 'DELETE', headers: { 'X-Admin-Key': key } });
  const data = await r.json();
  if (!r.ok || data.success === false) throw new Error(data.error || `HTTP ${r.status}`);
}
