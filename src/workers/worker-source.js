...
let 启用CF优化 = true; // CDN Fragment/TLS Optimizer (cf-optimizor) - خودکار برای هر لینک vless یک نسخه‌ی بهینه‌شده هم اضافه می‌کند
...
// ===================== CF Optimizor (cf-optimizor) Integration - START =====================
// منطق برگرفته از پروژه cf-optimizor: بهینه‌سازی لینک‌های vless برای دور زدن DPI با Fragment/TLS
// (آدرس CDN + fp=unsafe + Cipher Suites + FinalMask). فقط پارامترهای لازم عوض می‌شود؛ بقیه دست‌نخورده می‌ماند.
const CF优化_IP = '188.114.97.6';
const CF优化_FP = 'unsafe';
const CF优化_CS = ['TLS_AES_256_GCM_SHA384','TLS_CHACHA20_POLY1305_SHA256','TLS_AES_128_GCM_SHA256','TLS_ECDHE_ECDSA_WITH_AES_256_GCM_SHA384','TLS_ECDHE_RSA_WITH_AES_256_GCM_SHA384','TLS_ECDHE_ECDSA_WITH_AES_128_GCM_SHA256','TLS_ECDHE_RSA_WITH_AES_128_GCM_SHA256','TLS_ECDHE_ECDSA_WITH_CHACHA20_POLY1305_SHA256','TLS_ECDHE_RSA_WITH_CHACHA20_POLY1305_SHA256','TLS_ECDHE_ECDSA_WITH_AES_256_CBC_SHA','TLS_ECDHE_RSA_WITH_AES_256_CBC_SHA','TLS_ECDHE_ECDSA_WITH_AES_128_CBC_SHA256','TLS_ECDHE_RSA_WITH_AES_128_CBC_SHA256'].join(':');
const CF优化_FM = JSON.stringify({tcp:[{fragment:'tlshello',lengths:['5','94','1'],delays:['0'],maxSplit:'0'},{fragment:'1-1',lengths:['109','1'],delays:['1'],maxSplit:'355'}]});

function cf优化解析查询串(查询串) {
  const 结果 = [];
  if (!查询串) return 结果;
  查询串.split('&').forEach(项 => {
    if (!项) return;
    const 等号位置 = 项.indexOf('=');
    const 键 = 等号位置 < 0 ? 项 : 项.slice(0, 等号位置);
    const 值 = 等号位置 < 0 ? '' : 项.slice(等号位置 + 1);
    try { 结果.push({ k: decodeURIComponent(键), v: decodeURIComponent(值) }); } catch (e) { 结果.push({ k: 键, v: 值 }); }
  });
  return 结果;
}
function cf优化设置参数(参数列表, 键, 值) {
  const 小写键 = 键.toLowerCase();
  let 已找到 = false;
  for (let i = 参数列表.length - 1; i >= 0; i--) {
    if (参数列表[i].k.toLowerCase() === 小写键) {
      if (已找到) { 参数列表.splice(i, 1); continue; }
      参数列表[i] = { k: 键, v: 值 };
      已找到 = true;
    }
  }
  if (!已找到) 参数列表.push({ k: 键, v: 值 });
}
const CF优化_参数顺序 = ['cs','path','security','alpn','encryption','fm','insecure','host','fp','type','allowInsecure','sni'];
function cf优化拼接查询串(参数列表) {
  const 顺序表 = {};
  CF优化_参数顺序.forEach((k, i) => { 顺序表[k] = i; });
  const 甲 = [], 乙 = [];
  参数列表.forEach(项 => { (顺序表[项.k.toLowerCase()] !== undefined ? 甲 : 乙).push(项); });
  甲.sort((x, y) => 顺序表[x.k.toLowerCase()] - 顺序表[y.k.toLowerCase()]);
  return 甲.concat(乙).map(项 => encodeURIComponent(项.k) + '=' + encodeURIComponent(项.v)).join('&');
}
// یک لینک vless معمولی می‌گیرد و نسخه‌ی بهینه‌شده (CDN IP + fp=unsafe + cs + fm) را برمی‌گرداند. اگر vless نبود null.
function cf优化Vless(原始链接, 选项) {
  try {
    const 原始 = String(原始链接 || '').trim();
    if (!/^vless:\/\//i.test(原始)) return null;
    const 选项值 = 选项 || {};
    const 主体 = 原始.slice(8);
    let 剩余 = 主体, 片段 = '';
    const 井号位置 = 剩余.indexOf('#');
    if (井号位置 >= 0) { 片段 = 剩余.slice(井号位置 + 1); 剩余 = 剩余.slice(0, 井号位置); }
    let 查询串 = '';
    const 问号位置 = 剩余.indexOf('?');
    if (问号位置 >= 0) { 查询串 = 剩余.slice(问号位置 + 1); 剩余 = 剩余.slice(0, 问号位置); }
    const at位置 = 剩余.lastIndexOf('@');
    if (at位置 < 0) return null;
    const 用户ID = 剩余.slice(0, at位置);
    let 主机端口 = 剩余.slice(at位置 + 1);
    let 端口 = '';
    if (主机端口.charAt(0) === '[') {
      const 括号位置 = 主机端口.indexOf(']');
      if (括号位置 >= 0 && 主机端口.charAt(括号位置 + 1) === ':') 端口 = 主机端口.slice(括号位置 + 2);
    } else {
      const 冒号位置 = 主机端口.lastIndexOf(':');
      if (冒号位置 > 0) 端口 = 主机端口.slice(冒号位置 + 1);
    }
    const 参数列表 = cf优化解析查询串(查询串);
    cf优化设置参数(参数列表, 'fp', (选项值.fp || CF优化_FP));
    cf优化设置参数(参数列表, 'cs', (选项值.cs || CF优化_CS));
    cf优化设置参数(参数列表, 'fm', (选项值.fm || CF优化_FM));
    const 新地址 = 选项值.ip || CF优化_IP;
    const 新查询串 = cf优化拼接查询串(参数列表);
    return `vless://${用户ID}@${新地址}${端口 ? ':' + 端口 : ''}${新查询串 ? '?' + 新查询串 : ''}${片段 ? '#' + 片段 : ''}`;
  } catch (e) {
    return null;
  }
}
// از یک آرایه لینک (خروجی تولیدکننده‌های لینک ورکر) فقط لینک‌های vless را می‌گیرد و نسخه‌ی [CF] بهینه‌شده تولید می‌کند.
function cf优化链接列表(链接数组) {
  const 结果 = [];
  if (!Array.isArray(链接数组)) return 结果;
  for (const 链接 of 链接数组) {
    if (typeof 链接 !== 'string' || !/^vless:\/\//i.test(链接)) continue;
    const 优化后 = cf优化Vless(链接);
    if (!优化后) continue;
    const 井号位置 = 优化后.indexOf('#');
    const 主体 = 井号位置 >= 0 ? 优化后.slice(0, 井号位置) : 优化后;
    let 名称 = '';
    if (井号位置 >= 0) { try { 名称 = decodeURIComponent(优化后.slice(井号位置 + 1)); } catch (e) { 名称 = 优化后.slice(井号位置 + 1); } }
    名称 = (名称 ? 名称 + ' ' : '') + '[CF]';
    结果.push(`${主体}#${encodeURIComponent(名称)}`);
  }
  return 结果;
}
// صفحه‌ی ابزار دستی cf-optimizor (پیست لینک -> بهینه‌سازی) که مستقیم از خود ورکر سرو می‌شود.
const CF优化面板HTML = atob('');
// ===================== CF Optimizor (cf-optimizor) Integration - END =====================
  ...
  async fetch(请求735, 本地值734, 本地值733) {
    try {
      ...
      if (请求735.method === 'GET') {
        ...
        // ابزار دستی CF Optimizor: /cfopt (پیست لینک vless -> نسخه‌ی بهینه‌شده با CDN/Fragment/TLS)
        if (网址698.pathname === '/cfopt' || 网址698.pathname === '/cfopt/') {
          return new Response(CF优化面板HTML, {
            headers: {
...
async function 处理订阅请求(请求507, 用户506, 网址505 = null) {
  ...
  if (启用CF优化 && 最终链接列表.length > 0) {
    最终链接列表.push(...cf优化链接列表(最终链接列表));
  }
  if (最终链接列表.length === 0) {
    const 错误备注 = "所有节点获取失败";
