/* ============================================================
   Deploy Panel — Deploy Wizard  (static, runs on Cloudflare Pages)
...
function api(method, path, body, extraHeaders) {
  ...
  // Always use same-origin proxy at root level (/__cf/) for maximum compatibility
  // This eliminates CORS issues entirely since both frontend and proxy share the same origin
  var apiBase = '/__cf/';
...
function deployPages(code, kvId, d1Id, name, acc){
  ...
  .then(function(){
    ...
    var fd = new FormData();
    // Cloudflare's Pages deployment endpoint now REQUIRES a "manifest" field
    // (error 8000096: 'A "manifest" field was expected in the request body but was not provided.')
    // Since we're not uploading any static assets (only the _worker.js function), an empty
    // manifest object tells Cloudflare there are no asset files to map.
